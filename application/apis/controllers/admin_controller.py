...
from application.apis.businesslogic.admin_actions import create_admin_action, get_admins_all_action, delete_admin_action

router = APIRouter()

@router.post("/api/admins/create")
def create_admin_controller(admin: AdministratorSchema, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
...
def delete_one_controller(identifier:IdentifierEntitySchema, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
